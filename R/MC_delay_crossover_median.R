...
#' @param HR_fun Pre specified HR function
#' @param alpha One-sided significance level
#' @param M Number of simulations (for each p)
#'
#' @export

MC_delay_crossover_median <- function(n_c = 100,
                                      n_e = 100,
                                      median_c_1 = c(5, 10),
                                      median_c_2 = 10,
                                      median_e = 15,
                                      ...
                                      M = 100)
{
  ...
  result <- data.table(median_c = median_c_1,
                       p_switched = rep(0, length(median_c_1)),
                       Z_logrank = rep(0, length(median_c_1)),
                       Z_weighted = rep(0, length(median_c_1)),
                       Z_FH = rep(0, length(median_c_1)),
                       power_logrank = rep(0, length(median_c_1)),
                       power_weighted = rep(0, length(median_c_1)),
                       power_fh = rep(0, length(median_c_1)),
                       rel_eff_MWLRLR = rep(0, length(median_c_1)),
                       rel_eff_MWLRFH = rep(0, length(median_c_1)),
                       eff_MWLRLR = rep(0, length(median_c_1)),
                       eff_MWLRFH = rep(0, length(median_c_1)))

  for (i in 1:length(median_c_1)) {


    print(paste0("Simulation progress: ", i, " out of ", length(median_c_1)))
    # model parameters
    trt_switch_model <- list(n_c = n_c,
                             n_e = n_e,
                             median_c_1 = median_c_1[i],
